...
public class StatementAplicativo {
    ...
    public static boolean setLeituraProcesso(int codUsuario, int codProcesso) throws SQLException {
        ...
        query.setString(3, LocalDateTime.now().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.SHORT)));
        return query.execute();
    }
    ...
    public static boolean insereUsuario(int codUsuario) throws SQLException {
        String sql = "INSERT INTO perfil_usuario (cod_usuario, minutos_trabalho, minutos_jogos, minutos_social, minutos_outros) VALUES (?, ?, ?, ?, ?)";
        PreparedStatement query = new Conexao().getConexao().prepareStatement(sql);
        query.setInt(1, codUsuario);
        query.setInt(2, 0);
        query.setInt(3, 0);
